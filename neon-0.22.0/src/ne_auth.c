...
{
    return (*sess->reqcreds)(sess->reqcreds_udata, sess->realm, sess->attempt++,
			    sess->username, pwbuf);
}
...
static int parse_domain(auth_session *sess, const char *domain) {
    ...
    doms = split_string_c(domain, ' ', NULL, " \r\n\t", &count);
    if (doms != NULL) {
	if (count > 0) {
	    sess->domain = doms;
...
static int tokenize(char **hdr, char **key, char **value, 
		    int ischall)
{
    char *pnt = *hdr;
...
static void register_hooks(ne_session *sess, auth_session *as, const char *id)
{
    ne_hook_create_request(sess, ah_create, as);
    ne_hook_pre_send(sess, ah_pre_send, as);
    ne_hook_post_send(sess, ah_post_send, as);
    ne_hook_destroy_request(sess, ah_destroy, as);
    ne_hook_destroy_session(sess, free_auth, as);

    ne_set_session_private(sess, id, as);
}

static void auth_register(ne_session *sess,
			  const struct auth_class *ahc, const char *id, 
			  ne_request_auth callback, void *userdata) 
{
    auth_session *ahs = ne_calloc(sizeof *ahs);

    ahs->reqcreds = callback;
    ahs->reqcreds_udata = userdata;
    ahs->sess = sess;
    ahs->spec = ahc;

    register_hooks(sess, ahs, id);
}

void ne_set_server_auth(ne_session *sess, ne_request_auth callback, 
			void *userdata)
{
    auth_register(sess, &ah_server_class, HOOK_SERVER_ID, callback, userdata);
}

void ne_set_proxy_auth(ne_session *sess, ne_request_auth callback, 
		       void *userdata)
{
    auth_register(sess, &ah_proxy_class, HOOK_PROXY_ID, callback, userdata);
}
