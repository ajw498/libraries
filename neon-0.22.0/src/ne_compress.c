...
/* Process extra 'len' bytes of 'buf' which were recieved after the
 * DEFLATE data. */
static void process_footer(ne_decompress *ctx, 
			   const unsigned char *buf, size_t len)
{
    if (len + ctx->footcount > 8) {
	ne_set_error(ctx->session, "Too many bytes after data");
	ctx->state = NE_Z_ERROR;
    } else {
...
int ne_decompress_destroy(ne_decompress *ctx)
{
    ...
    if (ctx->state != NE_Z_PASSTHROUGH && ctx->state != NE_Z_BEFORE_DATA) {
