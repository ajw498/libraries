...
static int aborted(ne_request *req, const char *doing, ssize_t code)
{
    ...
    NE_DEBUG(NE_DBG_HTTP, "Aborted request (%d): %s\n", code, doing);
	...
	if (sess->socket != NULL) {
		...
		ne_set_error(sess, _("%s: socket error."), doing);
	    }
	} else {
	    char err[200];
...
static void add_fixed_headers(ne_request *req) 
{
    ...
    if (VERSION_PRE11(req->session) && !req->session->use_proxy) {
...
static void set_request_uri(ne_request *req, const char *uri)
{
    ne_buffer *real_uri = ne_buffer_create();
    req->abs_path = ne_strdup(uri);
    if (req->session->use_proxy && strcmp(uri, "*") != 0)
	ne_buffer_concat(real_uri, req->session->scheme, "://", 
			      ...
			      const char *method, const char *uri) 
{
    ...
    if (uri) set_request_uri(req, uri);
			       ...
			       char *buffer, size_t *buflen) 
{
	...
	*buflen = 0;
    ...
    if (readlen == NE_SOCK_CLOSED && resp->mode == R_TILLEOF) {
...
static ne_buffer *build_request(ne_request *req) 
{
    ...
    /* Use absoluteURI if speaking through a proxy, else abs_path */
    uri = req->session->use_proxy ? req->uri : req->abs_path;
...
static int read_status_line(ne_request *req, ne_status *status, int retry)
{
    ...
    if (status->reason_phrase) free(status->reason_phrase);
...
static int send_request(ne_request *req, const ne_buffer *request)
{
    ...
    if (ret < 0) {
	aret = aborted(req, _("Could not send request"), ret);
	return RETRY_RET(retry, ret, aret);
    }
	...
	if (ret < 0) {
	    aret = aborted(req, _("Could not send request body"), ret);
...
static int lookup_host(ne_session *sess, struct host_info *info)
{
    ...
    if (ne_name_lookup(info->hostname, &info->addr)) {
	ne_set_error(sess, _("Host `%s' not found"), info->hostname);
	return NE_LOOKUP;
    } else {
	return NE_OK;
    }
...
int ne_begin_request(ne_request *req)
{
	...
	(req->body_size > HTTP_EXPECT_MINSIZE) && !VERSION_PRE11(req->session);
    ...
    req->session->version_major = req->status.major_version;
    req->session->version_minor = req->status.minor_version;
    ne_set_error(req->session, "%d %s", 
		 req->status.code, req->status.reason_phrase);

    /* persistent connections supported implicitly in HTTP/1.1 */
    if (!VERSION_PRE11(req->session)) req->can_persist = 1;
    ...
    if (req->session->in_connect && req->status.klass == 2) {
    ...
    if (req->method_is_head || req->status.code==204 || req->status.code==304)
    	req->resp.mode = R_NO_BODY;
...
/* initalize socket. */
static void init_socket(ne_session *sess)
{
    if (sess->progress_cb) {
	ne_register_progress(sess->socket, sess->progress_cb,
			       sess->progress_ud);
    }
    if (sess->rdtimeout)
...
static int open_connection(ne_request *req) 
{
	...
	case 0:
	    ...
	    sess->socket = ne_sock_connect(sess->proxy.addr, sess->proxy.port);
	    if (sess->socket == NULL) {
		aborted(req, _("Could not connect to proxy server"),
			NE_SOCK_ERROR);
		return NE_CONNECT;
	    }
	...
	sess->socket = ne_sock_connect(sess->server.addr, sess->server.port);
	    
	if (sess->socket == NULL) {
	    aborted(req, _("Could not connect to server"), NE_SOCK_ERROR);
	    return NE_CONNECT;
	}
