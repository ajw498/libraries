...
{
    if (ne_name_lookup("localhost", &lh_addr))
	return FAILHARD;
...
int server_send(ne_socket *sock, const char *str, size_t len)
{
...
int discard_body(ne_socket *sock)
{
	...
	ONV(ret < 0, 
	    ("socket read failed (%d): %s", ret, ne_sock_error(sock)));
	clength -= ret;
