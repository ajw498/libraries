...
static int parse(void)
{
	...
	ONV(strcmp(res.scheme, exp->scheme),
	    ("%s: parsed scheme was %s not %s", exp->uri,
	     res.scheme, exp->scheme));
	ONV(strcmp(res.host, exp->host),
	    ("%s: parsed host was %s not %s", exp->uri, res.host, exp->host));
