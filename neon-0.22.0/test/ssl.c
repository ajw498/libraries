...
static int serve_ssl(ne_socket *sock, void *ud)
{
    ...
    CALL(do_ssl_response(sock, server_ctx, NULL, DEF_RESP));
...
static int fail_serve(ne_socket *sock, void *ud)
{
    ...
    do_ssl_response(sock, server_ctx, NULL, DEF_RESP);
...
static int simple(void)
{
    ...
    CALL(any_ssl_request(sess, serve_ssl, SERVER_CERT, CA_CERT, NULL, NULL));
    ne_session_destroy(sess);
...
static int serve_scache(ne_socket *sock, void *ud)
{
    ...
    CALL(do_ssl_response(sock, args->ctx, &sess, DEF_RESP));
 ...
 *
 * 2818: check subjectAltName THEN commonName in DN.
 * 2818: MUST check that commonName is most local RDN in subject DN.
 */
