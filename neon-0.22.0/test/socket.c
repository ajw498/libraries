...
static int init_ssl(void)
{
	...
	!SSL_CTX_load_verify_locations(client_ctx, "ca/cert.pem", NULL));
...
static int begin(ne_socket **sock, server_fn fn, void *ud)
{
    ...
    *sock = ne_sock_connect(localhost, 7777);
    ...
    ONV(ne_sock_use_ssl_os(*sock, client_ctx, NULL, NULL, NULL),
...
#define TO_BEGIN ne_socket *sock; to_begin(&sock)
...
static int ssl_closure(void)
{
	...
	("write got %d not reset or closure", ret));
...
static int read_reset(void)
{
    ...
    ONV(ret != NE_SOCK_RESET, ("read got %d not reset", ret));
    return good_close(sock);
}    
#endif
