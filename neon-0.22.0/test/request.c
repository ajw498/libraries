...
#define FOLLOW_REQ RESP200 "Content-Length: 0\r\n\r\n"
...
{
    ne_session *sess = ne_session_create("http", "localhost", 7777);
    ne_request *req = ne_request_create(sess, method, "/foobar");
    ssize_t ret;
    char *r = ne_malloc(strlen(resp) + sizeof(FOLLOW_REQ));
...
static int is_alive(int port)
{
    ...
    ne_name_lookup("localhost", &addr);
    sock = ne_sock_connect(addr, 7777);
    if (sock == NULL)
	return 0;
    else {
	ne_sock_close(sock);
