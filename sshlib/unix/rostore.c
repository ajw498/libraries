...
enum {
    ...
    INDEX_SESSIONDIR, INDEX_SESSION, INDEX_ROOTDIR,
};

enum {
    CHOICES_READ, CHOICES_WRITE,
};
...
/* PuTTY provides filenames with Unix naming conventions
 * (eg chiark.greenend.org.uk/ ).  We need to convert just the leafnames
 * to RISC OS form
 */
 /*
static void filename_unix_to_riscos(char *infile, char *outfile, int outlen)
{
  int i,l;
  ...
  for (i=0; i<l; i++)
    outfile[i]=__filename_char_map(infile[i]);

  outfile[l-1]='\0';
}
*/
static void make_filename(char *filename, int index, const char *subname, int read)
{
    char *home;
    int len;


    if (read==CHOICES_READ)
    {
      //home = getenv("Choices$Write");
      strncpy(filename, "Choices:", FILENAME_MAX);
    }
    else
    {
      strncpy(filename, "<Choices$Write>.", FILENAME_MAX);
    }
    len = strlen(filename);
    if (index == INDEX_SESSION) {
        char *munged = mungestr(subname);
        char *fn = dupprintf("Crypto.PuTTY.sessions.%s", munged);
        //char subnamero[FILENAME_MAX];
        strncpy(filename + len, fn, FILENAME_MAX - len);
        sfree(fn);
        sfree(munged);
    } else {
        strncpy(filename + len,
                index == INDEX_ROOTDIR ? "Crypto" :
                index == INDEX_DIR ? "Crypto.PuTTY" :
                index == INDEX_SESSIONDIR ? "Crypto.PuTTY.sessions" :
                index == INDEX_HOSTKEYS ? "Crypto.PuTTY.sshhostkeys" :
                index == INDEX_HOSTKEYS_TMP ? "Crypto.PuTTY.sshhostkeys/tmp" :
                index == INDEX_RANDSEED ? "Crypto.PuTTY.randomseed" :
                "Crypto.PuTTY.ERROR", FILENAME_MAX - len);
    }
...
void *open_settings_w(const char *sessionname, char **errmsg)
{
    ...
    *errmsg = NULL;
     ...
     * to catch it now.
     */
    make_filename(filename, INDEX_ROOTDIR, sessionname, CHOICES_WRITE);
    mkdir(filename, 0700);
    make_filename(filename, INDEX_DIR, sessionname, CHOICES_WRITE);
    mkdir(filename, 0700);
    make_filename(filename, INDEX_SESSIONDIR, sessionname, CHOICES_WRITE);
    mkdir(filename, 0700);

    make_filename(filename, INDEX_SESSION, sessionname,CHOICES_WRITE);
    fp = fopen(filename, "w");
    if (!fp) {
...
void *open_settings_r(const char *sessionname)
{
    ...
    make_filename(filename, INDEX_SESSION, sessionname, CHOICES_READ);
    fp = fopen(filename, "r");
    if (!fp)
	return NULL;		       /* can't open */
...
void *enum_settings_start(void)
{
    ...
    make_filename(filename, INDEX_SESSIONDIR, NULL,CHOICES_READ);
    dp = opendir(filename);
...
char *enum_settings_next(void *handle, char *buffer, int buflen)
{
    ...
    make_filename(fullpath, INDEX_SESSIONDIR, NULL, CHOICES_READ);
    len = strlen(fullpath);
		    ...
		    const char *keytype, const char *key)
{
    ...
    make_filename(filename, INDEX_HOSTKEYS, NULL, CHOICES_READ);
    fp = fopen(filename, "r");
    if (!fp)
		    ...
		    const char *keytype, const char *key)
{
     ...
     */
    make_filename(tmpfilename, INDEX_HOSTKEYS_TMP, NULL, CHOICES_WRITE);
    wfp = fopen(tmpfilename, "w");
    if (!wfp) {
        char dir[FILENAME_MAX];

	make_filename(dir, INDEX_ROOTDIR, NULL, CHOICES_WRITE);
	mkdir(dir, 0700);
        make_filename(dir, INDEX_DIR, NULL, CHOICES_WRITE);
        mkdir(dir, 0700);
        wfp = fopen(tmpfilename, "w");
    }
    if (!wfp)
    ...
    fclose(wfp);
    /* Now move the temporary filename into place - possibly different place
     * from where the filename we've just read came from
     */
    make_filename(filename, INDEX_HOSTKEYS, NULL, CHOICES_WRITE);
...
void read_random_seed(noise_consumer_t consumer)
{
    ...
    make_filename(fname, INDEX_RANDSEED, NULL, CHOICES_READ);
...
void write_random_seed(void *data, int len)
{
    ...
    make_filename(fname, INDEX_RANDSEED, NULL, CHOICES_WRITE);
    /*
