...
/* NB, control of 'addr' is passed via ssh_sk_new, which takes responsibility
 * for freeing it, as for new_connection() */
Socket ssh_sk_new(SockAddr addr, int port, int privport, int oobinline,
	      int nodelay, Plug p);

Socket ssh_sk_newlistener(char *srcaddr, int port, Plug plug, int local_host_only);
...
/*
 * Special error values are returned from sk_namelookup and ssh_sk_new
