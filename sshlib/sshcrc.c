 ...
 *
 * The ssh_crc32 standard polynomial is
 ...
 *    ssh_crc32_compute(buf,len) that uses it. Normal usage.
 *
 *  - Compile with INITFUNC defined. Will generate an uninitialised
 *    array as the table, and as well as ssh_crc32_compute(buf,len) it
 *    will also generate void ssh_crc32_init(void) which sets up the
 ...
 * init function.
 */
static unsigned long ssh_crc32_table[256];

void ssh_crc32_init(void)
{
 ...
 */
static const unsigned long ssh_crc32_table[256] = {
...
unsigned long ssh_crc32_update(unsigned long crcword, const void *buf, size_t len)
{
    const unsigned char *p = (const unsigned char *) buf;
    while (len--) {
	unsigned long newbyte = *p++;
	newbyte ^= crcword & 0xFFL;
	crcword = (crcword >> 8) ^ ssh_crc32_table[newbyte];
    }
    return crcword;
}

unsigned long ssh_crc32_compute(const void *buf, size_t len)
{
    return ssh_crc32_update(0L, buf, len);
}
