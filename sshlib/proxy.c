		      ...
		      const Config *cfg)
{
	 ...
	 */
	ret->sub_socket = ssh_sk_new(proxy_addr, cfg->proxy_port,
				 privport, oobinline,
				 nodelay, (Plug) pplug);
    ...
    return ssh_sk_new(addr, port, privport, oobinline, nodelay, plug);
}
		    ...
		    const Config *cfg)
{
    ...
    return ssh_sk_newlistener(srcaddr, port, plug, local_host_only);
}
